...
type SqlSectionProps = {
  dataset: LoadedDataset | null;
  analysisResult: DataQualityAnalysisResult | null;
};

export function SqlSection({ dataset, analysisResult }: SqlSectionProps) {
  ...
  if (!suggestion) {
    return (
      <SectionCard
        title="SQL"
        description="Exemplo de SQL sera sugerido com base no seu diagnostico."
      >
        <div className="rounded-2xl border border-dashed border-slate-500/60 bg-surface-900/60 p-6">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Carregue um arquivo para gerar um SQL de preparacao contextualizado com os
  ...
  return (
    <SectionCard
      title="SQL"
      description="Exemplo SQL generico de tratamento e preparacao orientado pelo diagnostico."
    >
