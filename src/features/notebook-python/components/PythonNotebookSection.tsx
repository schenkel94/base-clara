...
type PythonNotebookSectionProps = {
  dataset: LoadedDataset | null;
  analysisResult: DataQualityAnalysisResult | null;
};

export function PythonNotebookSection({ dataset, analysisResult }: PythonNotebookSectionProps) {
  ...
  const suggestion = useMemo(() => {
    ...
    return generatePandasNotebookSuggestion({
      dataset,
  ...
  if (!suggestion) {
    return (
      <SectionCard
        ...
        description="Codigo de exemplo com pandas sera sugerido com base no seu diagnostico."
      >
        <div className="rounded-2xl border border-dashed border-slate-500/60 bg-surface-900/60 p-6">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Carregue um arquivo para gerar um roteiro de limpeza em Python contextualizado com os
  ...
  return (
    <SectionCard
      title="Notebook Python"
      description="Exemplo de notebook pandas gerado a partir das caracteristicas do seu arquivo."
    >
      <div className="space-y-4">
        ...
        <div className="grid gap-2 md:grid-cols-2">
          {suggestion.steps.map((step) => (
            <article key={step.id} className="rounded-xl border border-surface-600 bg-surface-900/70 p-3">
